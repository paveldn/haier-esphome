...
template<typename... Ts> class DisplayOnAction : public Action<Ts...> {
 public:
  ...
  void play(const Ts &...x) override { this->parent_->set_display_state(true); }
...
template<typename... Ts> class DisplayOffAction : public Action<Ts...> {
 public:
  ...
  void play(const Ts &...x) override { this->parent_->set_display_state(false); }
...
template<typename... Ts> class BeeperOnAction : public Action<Ts...> {
 public:
  ...
  void play(const Ts &...x) override { this->parent_->set_beeper_state(true); }
...
template<typename... Ts> class BeeperOffAction : public Action<Ts...> {
 public:
  ...
  void play(const Ts &...x) override { this->parent_->set_beeper_state(false); }
...
template<typename... Ts> class VerticalAirflowAction : public Action<Ts...> {
 public:
  ...
  void play(const Ts &...x) override { this->parent_->set_vertical_airflow(this->direction_.value(x...)); }
...
template<typename... Ts> class HorizontalAirflowAction : public Action<Ts...> {
 public:
  ...
  void play(const Ts &...x) override { this->parent_->set_horizontal_airflow(this->direction_.value(x...)); }
...
template<typename... Ts> class HealthOnAction : public Action<Ts...> {
 public:
  ...
  void play(const Ts &...x) override { this->parent_->set_health_mode(true); }
...
template<typename... Ts> class HealthOffAction : public Action<Ts...> {
 public:
  ...
  void play(const Ts &...x) override { this->parent_->set_health_mode(false); }
...
template<typename... Ts> class StartSelfCleaningAction : public Action<Ts...> {
 public:
  ...
  void play(const Ts &...x) override { this->parent_->start_self_cleaning(); }
...
template<typename... Ts> class StartSteriCleaningAction : public Action<Ts...> {
 public:
  ...
  void play(const Ts &...x) override { this->parent_->start_steri_cleaning(); }
...
template<typename... Ts> class PowerOnAction : public Action<Ts...> {
 public:
  ...
  void play(const Ts &...x) override { this->parent_->send_power_on_command(); }
...
template<typename... Ts> class PowerOffAction : public Action<Ts...> {
 public:
  ...
  void play(const Ts &...x) override { this->parent_->send_power_off_command(); }
...
template<typename... Ts> class PowerToggleAction : public Action<Ts...> {
 public:
  ...
  void play(const Ts &...x) override { this->parent_->toggle_power(); }
